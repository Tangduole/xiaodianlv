...
import {
  ...
  X, Smartphone, FolderOpen, Upload, Zap, AlertCircle, ExternalLink,
} from 'lucide-react'
...
export default function App() {
  ...
  const handleUrlChange = (value: string) => {
    setUrl(value)
    if (value.trim()) { setDetected(detectPlatform(value)); } else { setDetected('') }
  }
  ...
  const clip = async (text: string, id: string) => { try { await navigator.clipboard.writeText(text); setCopied(id); setTimeout(() => setCopied(null), 3000) } catch {} }
  ...
  const detectedPlatformLabel = (id: string) => PLATFORMS.find(p => p.id === id)?.label || ''
      ...
      <div className="relative">
        ...
        <main className="max-w-2xl mx-auto px-6 pb-10">
          <div className="bg-slate-800/60 backdrop-blur-sm rounded-3xl p-6 border border-slate-700/60 shadow-xl">
              ...
              <div className="relative">
                ...
                {detected && (
                  <div className="absolute right-4 top-1/2 -translate-y-1/2 px-2 py-1 bg-orange-500/15 text-orange-300 text-xs rounded-lg border border-orange-500/20">
                    {detectedPlatformLabel(detected) || detected}
                  </div>
                )}
              </div>
            </div>
            ...
            {/* 保存位置 */}
            <div className="mb-5 flex items-center justify-between px-4 py-3 bg-slate-900/40 rounded-xl border border-slate-700/40">
              <span className="text-xs text-slate-500">保存至</span>
            ...
            <div className="mb-5 flex items-center justify-between px-4 py-3 bg-slate-900/40 rounded-xl border border-slate-700/40">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-slate-500">上传 Cookie</span>
